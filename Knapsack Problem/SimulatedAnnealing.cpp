...
Solution SimulatedAnnealing::run(unsigned maxWeight)
{
	...
	for (unsigned i = 0; i < kTimesToDecreaseTemp; i++) {
		...
		for (unsigned j = 0; j < kNeighbours; j++) {
			...
			else if(candidate.value() > 0){//				If you want to implement a discard method. Ignores solutions found if they are returned as invalid(-10) due to weight exceeding max
